#!/bin/bash  # shebang: يحدد مفسر الشل ليكون bash.

# سطر فارغ للفصل بين تعريف المفسر والتهيئة.
set -euo pipefail  # إنهاء السكربت فور حدوث أي خطأ (set -e), وحظر استخدام متغيرات غير معرّفة (set -u), وكشف أخطاء في سلاسل الأنابيب (pipefail).

# سطر فارغ للفصل بين الإعدادات.
: "${CI:=false}"  # تعيين المتغير CI إلى false إذا لم يكن معرفًا مسبقًا.
: "${WITH_REDIS:=false}"  # تعيين المتغير WITH_REDIS إلى false إذا لم يكن معرفًا، لخيارات إعداد Redis.
: "${SUDO_USER:=""}"  # تعيين متغير SUDO_USER إلى سلسلة فارغة إذا لم يكن معرفًا.

# سطر فارغ للفصل بين المتغيرات وتعريف الألوان.
NO_COLOR=''  # متغير لتعطيل الألوان في المخرجات بشكل افتراضي.
RED=''      # متغير لون أحمر للمخرجات.
CYAN='\033[0;36m'  # تسلسل ANSI لتفعيل اللون السيان.
GREEN='\033[0;32m'  # تسلسل ANSI لتفعيل اللون الأخضر.

# سطر فارغ قبل بدء التحقق من دعم الألوان.
# Check if terminal supports colors https://unix.stackexchange.com/a/10065/642181
if [ -t 1 ]; then  # إذا كان المخرج القياسي متصلًا بواجهة تيرمينال.
    total_colors=$(tput colors)  # جلب عدد الألوان المدعومة من الطرفية.
    if [[ -n "$total_colors" && $total_colors -ge 8 ]]; then  # إذا كانت الطرفية تدعم 8 ألوان أو أكثر.
        # https://stackoverflow.com/a/28938235/18954618  # مرجع لطريقة تعريف ألوان ANSI.
        NO_COLOR='\033[0m'  # تسلسل ANSI لإعادة الحالة الطبيعية للألوان.
        RED='\033[0;31m'    # تسلسل ANSI لتفعيل اللون الأحمر.
    fi
fi  # إغلاق شرط التحقق من دعم الألوان.

error_log() { echo -e "${RED}ERROR: $1${NO_COLOR}"; }  # دالة لعرض رسالة الخطأ باللون الأحمر.
info_log() { echo -e "${CYAN}INFO: $1${NO_COLOR}"; }  # دالة لعرض رسالة المعلومات باللون السيان.
error_exit() {  # دالة لعرض رسالة الخطأ وإنهاء السكربت.
    error_log "$*"  # عرض رسالة الخطأ.
    exit 1  # إنهاء السكربت بحالة خطأ.
}  # نهاية دالة error_exit.

# https://stackoverflow.com/a/18216122/18954618  # مرجع للتحقق من صلاحيات المستخدم.
if [ "$EUID" -ne 0 ]; then error_exit "Please run this script as root user"; fi  # تحقق من تشغيل السكربت كـ root.
detect_arch() {  # دالة لاكتشاف معمارية السي بي يو.
    case $(uname -m) in  # جلب المعمارية من uname.
    x86_64) echo "amd64" ;;  # إذا كانت المعمارية x86_64.
    aarch64 | arm64) echo "arm64" ;;  # إذا كانت arm64 أو aarch64.
    armv7l) echo "arm" ;;  # إذا كانت armv7l.
    i686 | i386) echo "386" ;;
    *) echo "err" ;;
    esac
}
